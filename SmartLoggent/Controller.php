...
class Piwik_SmartLoggent_Controller extends Piwik_Controller
{
	...
	private $availableLanguages = array(
			array("value" => "", "label" => ""),
			array("value" => "ita", "label" => "Italian"),
			array("value" => "eng", "label" => "English"),
			array("value" => "jpn", "label" => "Japanese"),
	);
	...
	public function overview()
	{
		...
		$view->availableLanguages = $this->availableLanguages;		
		$language = Piwik_Common::getRequestVar("language", "");
				
		$view->languageValue = $language;
		$filter = -1;
		if ($language) {
			$filter = array('SLLanguage'=>$language);
		}
	...
	public function searchPhrase()
	{
		...
		$view->searchPhrase = $this->getTable($fn,
				Piwik_SmartLoggent_API::INDEX_NB_QUERIES,
				20,
				$filter,
		...
		foreach ($this->array_metrics as $metric) {
			...
			$result_detail_metric_chart = $this->getCloud(Piwik_SmartLoggent_API::DIM_SEARCHPHRASE, 5, $filter, $metric);
			
			$result_detail_evolution_chart = $this->getGraph($fn, 
																Piwik_SmartLoggent_API::INDEX_NB_VISITS, 
																20, 
																$filter,
																Piwik_SmartLoggent_API::DIM_SEARCHPHRASE,
		...
		$view->searchPhraseEvolution = $this->getGraph($fn, 
															Piwik_SmartLoggent_API::INDEX_NB_VISITS, 
															20, 
															$filter,
															Piwik_SmartLoggent_API::DIM_SEARCHPHRASE,
		...
		$view->searchPhraseTagCloud = $this->getCloud(Piwik_SmartLoggent_API::DIM_SEARCHPHRASE, 5, $filter, Piwik_SmartLoggent_API::INDEX_NB_VISITS);
...
public function classes()
	{  
		...
		$view->class = $this->getTable($function,
				Piwik_SmartLoggent_API::INDEX_NB_VISITS,
				5, $filter, "classDatatable.tpl",
				Piwik_SmartLoggent_API::DIM_CLASS,
				'LOC_SL_Column_Label_Class');
		...
		foreach ($this->array_metrics as $metric) {
			...
			$result_detail_evolution_chart = $this->getGraph($function,
					$metric,
					1,$filter,
					Piwik_SmartLoggent_API::DIM_CLASS,
					'graphEvolution');
			...
			$classMetrics[] = "<h2>".Piwik_Translate('LOC_SL_Chart_'.$dimension.'_by_'.$metric)."</h2>".
				$this->getTopChart($dimension, $metric, true, true);
		...
		$view->evolution = $this->getGraph($function,
				Piwik_SmartLoggent_API::INDEX_NB_VISITS,
				1,$filter, 
				Piwik_SmartLoggent_API::DIM_CLASS,
				'graphEvolution');
	...
	public function singleClasses() {
		...
		$filter = array_merge($filter, array('SLClass'=>$slid));
		
		$urlIndex = Piwik_Url::getCurrentQueryStringWithParametersModified(array('module' => 'CoreHome',
				'action' => 'index',
		));
		
		$urlSingleSearchPhrase = Piwik_Url::getCurrentQueryStringWithParametersModified(array('action' => 'singleSearchPhrase'));
		$view->singleSearchPhraseUrl = $urlIndex . "#" . substr($urlSingleSearchPhrase, 1);
	...
	public function subClasses() {
		...
		foreach ($this->array_metrics as $metric) {
			...
			$result_subClassMetrics  = $this->getGraph('getDataFiltered',
					$metric,
					5,
					array_merge($filter, array('SLSuperClass'=>$slid)),
					Piwik_SmartLoggent_API::DIM_CLASS,
					'graphVerticalBar'
			);
	...
	public function clustering()
	{
		...
		$can = Piwik_Common::getRequestVar("can", "");
		if (!$can) $can = $clusterAnalysis[0]['value'];
		$view->canValue = $can;
		...
		$view->clusters = $this->getTable($function,
				Piwik_SmartLoggent_API::INDEX_NB_VISITS,
				10,
				array_merge($filter, array("SLClusterAnalysis"=> "$can")),
				"clusterDatatable.tpl",
				Piwik_SmartLoggent_API::DIM_CLUSTER,
				'LOC_SL_Column_Label_Cluster');
		
		$clusterMetrics = array();
		$detailcharts = array();
		
		foreach ($this->array_metrics as $metric) {
			$result_clusterMetrics = $this->getGraph($function,
					$metric,
					4,
					array_merge($filter, array("SLClusterAnalysis"=> "$can")),
					Piwik_SmartLoggent_API::DIM_CLUSTER,
	...
	public function namedEntityType()
	{
		...
		$view->namedEntitiesTypes = $this->getTable($function,
				Piwik_SmartLoggent_API::INDEX_NB_VISITS,
				10,
				$filter,
				"namedEntitiesTypesDatatable.tpl",
				Piwik_SmartLoggent_API::DIM_NAMEDENTITYTYPE);
		
		foreach ($this->array_metrics as $metric) {
			$result_NEMetrics = $this->getGraph($function,
				$metric,
				5,
				$filter,
				Piwik_SmartLoggent_API::DIM_NAMEDENTITYTYPE,
		...
		$view->evolution = $result_detail_evolution_chart = $this->getGraph($function,
				Piwik_SmartLoggent_API::INDEX_NB_VISITS,
				5,
				$filter,
				Piwik_SmartLoggent_API::DIM_NAMEDENTITYTYPE,
	...
	public function searchWords() {
		...
		$view->searchwords = $this->getTable($function,
				Piwik_SmartLoggent_API::INDEX_NB_VISITS,
				20,
				$filter,
		...
		foreach ($this->array_metrics as $metric) {
			$result_metrics = $this->getGraph($function,
					$metric,
					5,
					$filter,
					Piwik_SmartLoggent_API::DIM_SEARCHWORD,
	...
	public function singleSearchWord() {
		...
		foreach ($this->array_metrics as $metric) {
			$result_metrics = $this->getGraph('getDataFiltered',
					$metric,
					4,
					array_merge($filter, array('SLSearchWord' => $searchWord_id)),
					Piwik_SmartLoggent_API::DIM_SEARCHPHRASE,
					'graphVerticalBar'
			);
	...
	public function getTable($function=-1, $metric=-1, $limit=5, $arFilters = -1, $template = -1, $dimension = -1, $label = -1)
	{
		...
		static $lb; if ($label != -1) $lb = $label;
		
		if (!$lb) $lb = 'LOC_SL_Column_Label_SearchPhrase';
		...
		if ($arFlt != -1)
			$dataTable = Piwik_SmartLoggent_API::$fn($idSite, $period, $date, $segment, $dim, $arFlt);
		else
		...
		$view->init($this->pluginName,  __FUNCTION__, 'SmartLoggent.getSearchPhrase');
		$view->setDatatable($dataTable);
		$result = $this->configureUsualTable($view, $lb, true, $limit, Piwik_SmartLoggent_API::INDEX_NB_QUERIES, "SmartLoggent/templates/$tpl");
	...
	public function getCloud($dimension = -1, $limit = 5, $arFilters = -1, $metric = -1) {
		...
		if ($arFlt)
			$dataTable = Piwik_SmartLoggent_API::getDataFiltered($idSite, $period, $date, $segment, $dim, $arFlt);
		else
