 ...
 */
class Piwik_SmartLoggent_Core_Archive_Array_IndexedByDate extends Piwik_Archive_Array
{
	/**
...
// 		$profiler = Piwik::profilestart(get_class($this).'::'.__FUNCTION__); // 		Piwik::profileend($profiler);
		...
		foreach($rangePeriod->getSubperiods() as $subPeriod)
		{
			...
			$archive = Piwik_SmartLoggent_Core_Archive::build($oSite->getId(), $strPeriod, $startDate, $segment->getString() );
