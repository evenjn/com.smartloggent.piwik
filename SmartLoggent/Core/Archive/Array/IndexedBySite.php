...
/**
 * @package Piwik
 * @subpackage Piwik_SmartLoggent_Core_Archive
 */
class Piwik_SmartLoggent_Core_Archive_Array_IndexedBySite extends Piwik_Archive_Array 
{
	/**
...
// 		Piwik::profileend($profiler);
	...
	public function getDataTableFromNumeric( $fields )
	{
		...
		if ($this->getFirstArchive() instanceof Piwik_Archive_Single || $this->getFirstArchive() instanceof Piwik_SmartLoggent_Core_Archive_Single)
		{
		...
		elseif ($this->getFirstArchive() instanceof Piwik_Archive_Array || $this->getFirstArchive() instanceof Piwik_SmartLoggent_Core_Archive_Array)
		{
			foreach($this->archives as $idSite => $archive)
			{
	...
	private function getArchiveIdsAfterLaunching( $metrics )
	{
		...
		{
			$report = Piwik_SmartLoggent_Core_Archive_Single::getRequestedReportFor($metric);
			...
			foreach($this->archives as $archive)
	 		{
				...
				{
					throw new Exception("Piwik_SmartLoggent_Core_Archive_Array_IndexedBySite::getDataTableFromNumeric() algorithm won't work if data is stored in different tables");
				}
	 		}
	 	}
